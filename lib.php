    ...
    if ($CFG->voiceshadow_convert == 1) {
        if ($data = $DB->get_record_sql("SELECT * FROM {voiceshadow_process} WHERE `status` = 'open' LIMIT 1")) {
            ...
            if ($data->type == 'audio/wav')
                $datasend = array('name' => $data->name, 'fullpatch' => $from->fullpatch, 'mconverter_wav' => '@' . $from->fullpatch);
        ...
        } else if ($dataall = $DB->get_records_sql("SELECT * FROM {voiceshadow_process} WHERE `status` = 'send' AND time > ?", array($timecheck))) {
...
function voiceshadow_sort_table_data($data, $titlesarray, $orderby = 'ASC', $sort, $page, $perpage)
{
    $j = 0;
    $orderkey = $j;
    if ($sort) {
...
function voiceshadow_cmp_phon($spoken, $target)
{
    ...
    foreach ($target_obj->phonetic as $index => $word) {
        if (in_array($word, $spoken_obj->phonetic)) {
            $score++;
            if (isset($target_obj->words[$index])) {
...
function voiceshadow_make_table_headers($titlesarray, $orderby, $sort, $link)
{
    ...
    foreach ($titlesarray as $titlesarraykey => $titlesarrayvalue) {
        ...
        if (!empty($titlesarrayvalue)) {
            $table[] = "<a href=\"" . $link . "&sort=$titlesarrayvalue&orderby=$columndir\">$titlesarraykey</a>$columnicon";
        } else {
            $table[] = "$titlesarraykey";
        }
    }
